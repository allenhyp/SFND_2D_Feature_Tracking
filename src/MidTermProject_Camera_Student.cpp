...
int main(int argc, const char *argv[])
{
    ...
    while (inputFileStream >> detectorType >> descriptorType)
    {
        // reset dataBuffer
        ...
        for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
        {
            ...
            if (bFocusOnVehicle)
            {
                vector<cv::KeyPoint> kptsFiltered;
                for (auto keypoint : keypoints)
                {
                    if (vehicleRect.contains(keypoint.pt))
                        kptsFiltered.push_back(keypoint);
                }
            ...
            cv::Mat descriptors;
            extractTime = descKeypoints((dataBuffer.end() - 1)->keypoints, (dataBuffer.end() - 1)->cameraImg, descriptors, descriptorType);
            // outputFileStream << descriptorType << " compute description time = " << extractTime << endl;
            totalDetAndExtTime += extractTime;
            //// EOF STUDENT ASSIGNMENT
            ...
            if (dataBuffer.size() > 1) // wait until at least two images have been processed
            {
                ...
                // outputFileStream << "[" << detectorType << ", " << descriptorType << "] matched " << matches.size() << "keypoints" << endl;
